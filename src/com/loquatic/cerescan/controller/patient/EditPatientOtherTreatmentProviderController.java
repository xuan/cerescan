...
public class EditPatientOtherTreatmentProviderController extends
		GenericForwardComposer {
	private static Log log = LogFactory
			.getLog(EditPatientOtherTreatmentProviderController.class);
	...
	public void lastNameSearch() {
		if (searchLastNameTb.getValue().length() < 3) {
			...
			if (physicians != null && physicians.size() > 0) {
				...
				errLbl.setValue("No treatment provider found for: "
	...
	public void save() {
		sessionInfo
				.setAlternateTreatmentProviders((List<Physician>) selectedPhysiciansLb
						.getModel());
	...
	public void onClick$deleteBtn(MouseEvent event) {
		try {
			String msg = "Do you want to delete this treatment provider from the patient's file? ";
