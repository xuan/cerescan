...
public class ComponentPrinter {
	 ...
	 * @param out
	 */
	static private void recursivePrint(Component comp, PrintStream out,
			int level) {
	...
	private static void dump(StringBuffer sb, Component comp) {
		if (comp instanceof ComponentPrinter)
			return;
		...
		if (true)
			return;
		sb.append("=").append(compCtrl.getAnnotations().size() + " : ").append(
				compCtrl.getAnnotatedProperties().size());
		String prop;
		...
		for (Iterator it = compCtrl.getAnnotatedProperties().iterator(); it
				.hasNext();) {
			prop = (String) it.next();
			sb.append(" ").append(prop).append(compCtrl.getAnnotations(prop));
		}
	}
}
